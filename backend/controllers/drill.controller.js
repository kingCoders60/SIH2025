...
export const getUserDrills = async (req, res) => {
  try {
    ...
    // ✅ Agar koi drills na mili to bhi empty array return ho raha hai.
    res.json(drills);
  } catch (err) {
    // ✅ Yeh sabse bada fix hai. Error hone par bhi ek empty array return karein.
    console.error("Error in getUserDrills:", err);
    res.status(500).json([]); // Ye front-end ko crash hone se bachayega
  }
};
